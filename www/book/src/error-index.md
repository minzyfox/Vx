...
| [`E6017`](/errors/E6017/) | A misuse of the `raw::` transfer-lowering primitives (Vx#353 A2): a `raw::` call outside an `impl transfer` body, an unknown primitive name, a tile argument that is not a bare parameter name (the primitives are indexed, not addressed), a store into a tile not held by `&mut`, or a wrongly typed index/value. |
...
| [`E6022`](/errors/E6022/) | An `impl transfer` lowering whose edge endpoints are not visible to a topology that declares the edge -- the lowering would execute on a part that cannot address the spaces it moves bytes between (contract constraint C6). |
